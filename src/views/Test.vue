...
const isAnimating = ref(false); // To disable controls during animation
...
const isSwapCorrect = computed(() => {
  if (selectedIds.value.length !== 2) return false;
  const id1 = actualMinInUnsorted.value?.id;
  const id2 = swapTargetId.value;
  return selectedIds.value.includes(id1!) && selectedIds.value.includes(id2!);
});
...
const coachHint = computed(() => {
  ...
  if (selectedIds.value.length < 2) {
    return `Selecciona dos números para intercambiar. (Pista: El mínimo es ${actualMinInUnsorted.value?.value})`;
  }
...
// --- Watcher for Auto-Advancing (only if swap was correct) ---
watch(sortedBoundaryIndex, () => {
  // Simple increment, logic is handled post-swap
});
...
function performAnimatedSwap() {
  if (selectedIds.value.length !== 2 || isAnimating.value) return;

  isAnimating.value = true;
  const wasCorrectSwap = isSwapCorrect.value;

  const indexA = numbers.value.findIndex(n => n.id === selectedIds.value[0]);
  const indexB = numbers.value.findIndex(n => n.id === selectedIds.value[1]);
...
<template>
  <div class="page-container">
    <div class="container">
      <h1 class="title">Demostración de Intercambio</h1>
      <p class="subtitle">Selecciona dos números y observa cómo se intercambian usando una variable temporal.</p>
      
      <div class="controls">
        <button v-if="!isPracticeStarted" @click="addNumber" class="btn btn-primary" :disabled="numbers.length >= 10">Añadir Número</button>
        <button v-if="!isPracticeStarted && numbers.length > 0" @click="startPractice" class="btn btn-accent">Empezar a Practicar</button>
        <button v-if="selectedIds.length === 2" @click="performAnimatedSwap" class="btn btn-accent" :disabled="isAnimating">Realizar Intercambio</button>
        <button @click="reset" class="btn btn-secondary" :disabled="isAnimating">Reiniciar</button>
      </div>
      ...
      <div class="main-area">
        <div class="numbers-container">
          <div
            ...
            :class="{ 
              ...
              'is-actual-min': !isAnimating && num.id === actualMinInUnsorted?.id
            }"
          >
        ...
        <div class="temp-area">
          ...
          <div 
            ...
            :class="{ 'has-value': (isAnimating ? animatedState.temp : false) }"
          >
      ...
      <div class="history-container" v-if="history.length > 0">
        <h2 class="history-title">Pasos del Intercambio</h2>
...
<style scoped>
/* Keeping styles mostly the same, but adjusting for new classes and states */
...
.subtitle { font-size: 1.1rem; color: var(--pink); margin-bottom: 2rem; }
.controls { min-height: 60px; margin-bottom: 1rem; display: flex; justify-content: center; align-items: center; gap: 1rem; }
...
.numbers-container { display: flex; flex-wrap: wrap; justify-content: center; gap: 1rem; min-height: 176px; max-width: 600px; }
.temp-area { display: flex; flex-direction: column; align-items: center; gap: 0.5rem; }
...
.btn { padding: 0.8rem 1.5rem; border: none; border-radius: 8px; font-size: 1rem; font-weight: 600; cursor: pointer; transition: all 0.3s ease; color: white; }
...
.number-wrapper.sorted { border-color: var(--sorted-color); opacity: 0.6; cursor: default; }
