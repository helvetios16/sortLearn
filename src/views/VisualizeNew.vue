  ...
  >
    ...
    <div class="container mx-auto px-4 py-2 h-[calc(100vh-85px)] flex flex-col gap-4">
      ...
      <div class="grid grid-cols-12 gap-2 h-[35%]">
        ...
        <!-- COMPLEJIDAD -->
        <div
          class="col-span-4 bg-white rounded-lg shadow-md p-2 border border-blue-200 transition-all duration-500 relative"
          :class="{
            'opacity-20 grayscale pointer-events-none':
        ...
        >
          ...
          <div class="flex flex-col gap-2 h-full">
            ...
            <div
              class="bg-white rounded border border-gray-200 p-1.5 flex-1 flex flex-col justify-start"
            >
                ...
                >
                  ...
                  <span class="font-mono text-[11px] whitespace-nowrap min-w-[30px] text-right">
                    <span
                      ...
                      class="font-extrabold text-blue-700 text-xs"
  ...
  >
    ...
    >
      <div class="text-center mb-6">
        ...
        <template v-else>
          <p class="text-gray-600 text-sm mt-2">
            Observa la sección de memoria resaltada. Usamos una variable para recordar el mínimo.
          </p>
          <p class="text-gray-800 font-bold mt-2 text-lg">
            ¿Cómo crece la MEMORIA EXTRA necesaria si aumenta la cantidad de datos (n)?
          </p>
          <p class="text-xs text-gray-500 mt-1 italic">
            (Nota: No cuentes el array original, solo el espacio adicional "extra")
          </p>
        </template>
      </div>
...
const handleMidQuizAnswer = (option: string) => {
  ...
  if (quizTopic.value === 'space') {
    if (option === 'o1') {
      ...
      midQuizFeedback.value = {
        type: 'error',
        text: 'Incorrecto. 💾 <br> El array original NO cuenta como espacio "extra". No estamos creando copias del array.',
      };
    } else if (option === 'on2') {
      midQuizFeedback.value = {
        type: 'error',
        text: 'Incorrecto. 💾 <br> Las comparaciones consumen tiempo (CPU), no memoria (RAM).',
      };
    }
    return;
  }
